...
| Upload transcript | Performed by `pj sync` | `PJ_TRANSCRIPT_BEGIN/WRITE/COMMIT`, with best-effort abort | `PUT /v1/transcripts/{id}` | Physical USB access or bearer token | USB transfer protocol or v0 transcript endpoint and an existing recording | Yes | Atomic commit requires current WAV SHA-256 and byte length; stale sources are retryable and missing/malformed provenance fails closed |
...
| Local note library | `pj library list/show/title/tui/serve` | Not a device operation | Not a device operation | Local user account | SQLite schema migrations and deletion tombstones | Local title and local-library deletion | Stable `(device_id, audio_id)` identity; imports legacy jobs idempotently; deleted local notes stay hidden until a real device sync rediscovers them |
| Terminal/browser library | `pj library tui/serve` | Not a device operation | Loopback HTTP only | Local user account plus per-process CSRF token for writes | Local library | Search, filter, title edit, playback, and confirmed local deletion | Browser audio uses validated identities and byte ranges; CSRF protects writes; non-loopback binds and DNS-rebinding host headers are refused |
...
identifiers are bounded to 95 UTF-8 bytes, audio chunks are capability-bounded to
1024 bytes with a 256-byte legacy fallback,
