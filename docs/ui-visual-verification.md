...
The output is written to `build/ui-gallery/`. `gallery.png` is a contact sheet for visual review; 23 individual PNGs cover primary menus, both clock/unit modes, 12-hour alarm disambiguation, recording, empty and populated Listen/Read lists, three-note paging controls and a second note page, playback and transcript detail, time tools, the flattened Settings rows in light/dark and 12/24-hour states, and the volume screen. `manifest.json` records pixel metrics for each frame.
